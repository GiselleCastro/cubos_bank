...
import { AppError, BadRequestError, InternalServerError, UnprocessableEntityError } from "../err/appError";
...
export class CreateUserUseCase {
  ...
  try{
    ...
    if(data.document.length == DocumentLength.CPF){
      documentAnalysis = await this.compilanceAPI.validateCPF({document: data.document})
    } else if (data.document.length == DocumentLength.CNPJ){
      documentAnalysis = await this.compilanceAPI.validateCNPJ({document: data.document})
    } else {
