...
export class CreateAccountUseCase {
  ...
  async execute(data: CreateAccountData, userId: string): Promise<CreateAccountReturn> {
    const accountRegister = await this.accountsRepository.findByAccountId(data.account);

    if (accountRegister) {
      throw new BadRequestError('Account already registered.');
    }
    
    const newAccount = await this.accountsRepository.create({id: uuid(), ...data, userId});
